...
; 目前阶段的理解：可以看到我们这里简单的把每个段的段基址都设置为了0，其实linux中也是这样，
;逻辑和线性地址的映射关系变得简单，linux中主要使用了分页基址
; GDT ------------------------------------------------------------------------------------------------------------------------------------------------------------
;                                           段基址        	段界限     , 	属性
...
BaseOfStack	equ	0100h ;访问栈是使用ss:ip,所以设置为0x0100即可

LABEL_START:
	mov	ax, cs  ;此时的cs已经为0x9000，还在实模式
	mov	ds, ax
	mov	es, ax
	mov	ss, ax
	mov	sp, BaseOfStack
	...
	mov dh, 2
	push dx		;这里是自己实现的，想要通过这样讲dh参数传递过去，控制显示在第几行

	mov	dh, 0			; "Loading  "选择显示字符串
...
SetupPaging:
	...
	xor	edx, edx
	mov	eax, [dwMemSize] ; 被除数
	mov	ebx, 400000h	; 400000h = 4M = 4096 * 1024, 一个PDE对应的内存大小，作为除数
	div	ebx
	mov	ecx, eax	; 此时 ecx 为 PDE 应该的个数，AX存储除法操作的商
	test	edx, edx ; DX存储除法操作的余数,test进行逻辑与操作，并将ZF标志位置位，但不改变源操作数
	jz	.no_remainder
	inc	ecx		; 如果余数不为 0 就需增加一个PDE
.no_remainder:
	push	ecx		; 暂存PDE个数

	; 为简化处理, 所有线性地址对应相等的物理地址. 并且不考虑内存空洞.

	; 首先初始化页目录，此时ecx已经为PDE的个数了
	mov	ax, SelectorFlatRW
	mov	es, ax
...
.1:
	stosd	;mov [edi]，eax；edi=edi+4；这里是在把PTE的地址填充到PDE表项中
	add	eax, 4096		; 为了简化, 所有页表在内存中是连续的.
	loop	.1

	; 再初始化所有页表
	pop	eax			; PDE个数
	mov	ebx, 1024		; 每个PDE 1024 个 PTE
	mul	ebx
...
.2:
	...
	mov	eax, PageDirBase ; 将CR3指向页目录的首地址
	mov	cr3, eax
	mov	eax, cr0
	or	eax, 80000000h	; 开启分页功能
	mov	cr0, eax
	jmp	short .3
.3:
	nop
...
InitKernel:
        xor   esi, esi
        mov   cx, word [BaseOfKernelFilePhyAddr+2Ch]; ecx <- pELFHdr->e_phnum，循环读取program header table的次数
        movzx ecx, cx                               
        mov   esi, [BaseOfKernelFilePhyAddr + 1Ch]  ; esi <- pELFHdr->e_phoff，program header table的文件偏移
        add   esi, BaseOfKernelFilePhyAddr;esi<-OffsetOfKernel+pELFHdr->e_phoff，文件偏移+文件在内存中的位置
.Begin:
        mov   eax, [esi + 0]
        cmp   eax, 0                ; program 的第一个成员是type，不能为0
        jz    .NoAction
        push  dword [esi + 010h]    ; 该 program 应该从文件中读取的大小 size ，为MemCpy的大小入栈 ： iSize
        mov   eax, [esi + 04h]      ; 该 program 在文件中的偏移 
        add   eax, BaseOfKernelFilePhyAddr;  文件偏移+文件在内存中的位置
        push  eax		    ;作为MemCpy的源入栈 ：src  
        push  dword [esi + 08h]     ; esi + 08h代表 program 应加载到的虚拟地址，作为 MemCpy的目的入栈 ：dst  
        call  MemCpy                      ; 将文件内容加载到指定的虚拟内存
        add   esp, 12                     ; 恢复堆栈
.NoAction:
        add   esi, 020h                   ; esi += pELFHdr->e_phentsize，继续找下一个program header
        dec   ecx
        jnz   .Begin
        ret
; InitKernel ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
_szkerChkTitle:			db	"init kernel sucess", 0Ah, 0
...
;
;; 保护模式下使用这些符号
szkerChkTitle		equ	BaseOfLoaderPhyAddr + _szkerChkTitle
